export const FAKE_CALLS = [
  {
    id: 'call-001',
    title: 'Technical Validation — Asteria Health',
    account: 'Asteria Health',
    stage: 'Technical Validation',
    arr: '$1.2M',
    date: '2026-02-20',
    duration: '52 min',
    participants: ['Sarah Chen (AE)', 'Marcus Webb (SE)', 'Dr. Lisa Park (CTO)', 'James O\'Brien (Architect)'],
    summary: 'Deep technical session validating platform\'s HTAP capabilities against Asteria\'s claims processing workload. CTO confirmed SingleStore is the primary competition. Team showed strong interest in columnar performance — ran live benchmark queries on their anonymized dataset. Key concern: 24/7 ops team managing schema migrations during business hours.',
    ...
    nextSteps: [
      'SE to send columnar replica vs ClickHouse benchmark report by Feb 24',
      'AE to loop in internal legal re: BAA template — this week',
      'Schedule POC kickoff for March 3, using their anonymized claims dataset',
      'Send managed cloud pricing for Managed Distributed SQL — Thursday',
    ],
    questions: [
      'How does platform handle online schema changes at 50K TPS without downtime?',
      'What are the SLA guarantees for Managed Distributed SQL in us-east-1?',
      'Can columnar replica handle ad-hoc aggregations on 3 years of claims data?',
      'What does the migration path from SingleStore look like?',
    ],
    collateral: [
      { title: 'columnar replica vs ClickHouse Benchmark (Healthcare)', url: '#' },
...
Marcus: Absolutely. So we took the 50-million-row anonymized claims sample you shared and loaded it into platform. What you're seeing on screen is a real-time OLTP workload — 45,000 transactions per second — while simultaneously running this aggregation query against columnar replica. Notice the OLTP latency doesn't budge. P99 stays under 8ms.
...
Marcus: Two storage engines in one cluster. row-store engine handles OLTP with Raft consensus — strong consistency. columnar replica is a columnar replica that replicates asynchronously. The optimizer routes automatically based on query type. No manual routing, no ETL pipeline.
...
Marcus: That's exactly what we'd propose. March 3rd POC kickoff, 30-day evaluation on Managed Distributed SQL in us-east-1. We'll provide a dedicated SE resource throughout.

Sarah: James, for the Kubernetes concern you raised — Managed Distributed SQL is fully managed. Your ops team doesn't touch k8s. We handle upgrades, failover, backups, the works.
...
Sarah: I'll send detailed pricing Thursday — but for your scale, Managed Distributed SQL in us-east-1, you're looking in the range of $80K-120K annually for the infrastructure, plus the licensing. Well within your $1.2M initial conversation.`,
  },
  {
    id: 'call-002',
    ...
    summary: 'POC design session for claims data platform modernization. Team is evaluating platform against MySQL + Redshift current stack. Rachel expressed concern about DBA-free operations — current team relies heavily on manual tuning. Strong HIPAA discussion — Northwind processes 2M claims/day. POC agreed: 60-day evaluation starting March 1.',
    ...
    risks: [
      ...
      'AWS-centric org — Managed Distributed SQL on AWS required, GCP not acceptable',
    ],
    nextSteps: [
      'Send POC success criteria document by Feb 22',
      'Confirm AWS us-east-1 region availability for POC cluster',
      'Schedule DBA team intro call — include platform compatibility talking points',
      'Rachel to share Redshift schema for migration assessment',
    ],
    questions: [
      'How does platform\'s query optimizer compare to Redshift for analytics?',
      'Can platform replace both MySQL and Redshift with a single cluster?',
      'What tooling exists for migrating from Redshift?',
      'How does auto-scaling work on Managed Distributed SQL?',
    ],
    collateral: [
      { title: 'MySQL to platform Migration Playbook', url: '#' },
      { title: 'platform vs MySQL + Redshift Architecture Comparison', url: '#' },
      { title: 'Managed Distributed SQL Auto-Scaling Overview', url: '#' },
    ],
...
Tom: Sounds good. My main question: can platform actually replace both MySQL AND Redshift? Because if it can, that's a massive operational simplification for us.

Priya: That's exactly our pitch. Today you have MySQL handling OLTP — claims intake, member portal — and Redshift handling analytics and reporting. With platform HTAP, that's one cluster, one ops burden, one billing relationship.

Rachel: My concern is the analytics performance. We have some heavy aggregation queries — claims cohort analysis, readmission rates. Those are finely tuned in Redshift. columnar replica needs to match or beat that.

Priya: Fair. For the POC we'd propose loading your 90-day claims sample — what's that, roughly 180 million rows? — into columnar replica and running your five critical reports head-to-head. We'll document query times, resource utilization, and output validation.

Tom: That works. What about the MySQL side? Our DBA team has 15 years of MySQL tuning experience. They're skeptical.

Priya: platform is MySQL-protocol compatible. Same connection string, same client tools, same explain output format. Their skills transfer directly. We're not asking them to relearn — we're asking them to add one skill: horizontal scaling without sharding.
...
Priya: Read access to a Redshift snapshot for migration assessment, your top 5 analytics queries for benchmarking, and a VPC peering setup from your AWS account to Managed Distributed SQL. We'll handle everything else.
...
Alex: Perfect. I'll send the formal POC success criteria doc by Monday. Once we're both signed off on that, we're go for March 1.`,
  },
  {
    ...
    risks: [
      ...
      'Multi-region requirement — need Managed Distributed SQL multi-region GA confirmation',
    ],
    nextSteps: [
    ...
    questions: [
      'What is the migration path from 47 MySQL shards to platform?',
      'How does platform multi-region compare to CockroachDB multi-region?',
      'What is the typical ops headcount reduction after platform adoption?',
      'Can platform handle Black Friday traffic spikes without pre-sharding?',
    ],
    collateral: [
      { title: 'MySQL Sharding to platform Migration Guide', url: '#' },
      { title: 'platform vs CockroachDB Comparison', url: '#' },
      { title: 'Retail Customer Case Studies', url: '#' },
    ],
    transcript: `Jordan: Diana, Sam — thank you for making time. Today we want to present the business case for platform at Summit, specifically the TCO analysis and the operational impact story.

Diana: Before you start — I need to understand the ROI on headcount. I've got three engineers whose primary job is managing MySQL shards. If platform eliminates that, I need to see that in numbers.
...
Kevin: With platform, you have one logical database. Horizontal scaling is automatic — platform adds nodes when utilization hits threshold. No manual resharding, no cross-shard query logic. Your engineers shift from shard wrangling to feature development.

Diana: What's the infrastructure cost comparison?

Kevin: We ran the numbers against your current AWS spend that Jordan shared. Today you're running 47 RDS MySQL instances, plus the EC2 for your application-layer aggregation. That's approximately $380K annually. Managed Distributed SQL for your workload — validated against your peak QPS — is approximately $220K. That's a $160K direct infrastructure saving in year one.
...
Kevin: Managed Distributed SQL auto-scaling. You define your min and max row-store engine node count. During Black Friday you scale out within minutes — no pre-provisioning required. Scale back down after. You only pay for what you use.

Diana: The CockroachDB team made similar claims. What's different?

Kevin: CockroachDB uses a PostgreSQL wire protocol — your MySQL applications need code changes. platform is MySQL-compatible — same connection string, no code changes. For a migration from 47 MySQL shards, that matters enormously.
...
export const FAKE_PRIORITIES = [
  { account: 'Asteria Health', stage: 'Technical Validation', value: '$1.2M', risk: 'BAA legal delay 3wk' },
...
export const FAKE_COACHING = [
  { account: 'Asteria Health', happened: 'CTO engaged on columnar replica live benchmark — strong pull. BAA blocker identified.', next: 'Send BAA template today, schedule legal intro call by Friday.' },
