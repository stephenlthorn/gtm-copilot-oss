...
export async function GET(request) {
  ...
  const account = (query.searchParams.get('account') || '').trim();
  const topK = (query.searchParams.get('top_k') || '10').trim();

  if (!q || q.length < 2) {
    return Response.json({ query: q, results: [] }, { status: 200 });
  }

  const upstream = new URL(`${API_BASE}/kb/search`);
  upstream.searchParams.set('q', q);
  upstream.searchParams.set('top_k', topK);
  if (sourceType) upstream.searchParams.set('source_type', sourceType);
  if (account) upstream.searchParams.set('account', account);
