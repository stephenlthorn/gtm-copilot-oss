...
OUTPUT_PATH = OUTPUT_DIR / "gtm-copilot-gm-brief.pptx"
LOGO_PATH = ROOT / "ui" / "public" / "logo.svg"
...
def slide_1_overview(prs: Presentation):
    ...
    add_header(slide, "GTM Copilot", "Internal GTM copilot for answers, recommendations, and asset generation")
...
def slide_2_single_llm_mcp(prs: Presentation):
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    style_background(slide)
    add_header(slide, "Simple Architecture: One Enterprise LLM + MCP", "Single-model path with internal platform knowledge grounding")
    ...
    add_bullets(
        slide,
        [
            "MCP server fronts the internal platform knowledge DB (docs + approved call transcripts).",
...
def slide_3_outputs(prs: Presentation):
    ...
    add_bullets(
        ...
        [
            ...
            "Internal-only access via OAuth/SSO; outbound messages restricted to @example.com.",
