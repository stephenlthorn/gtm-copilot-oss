#!/usr/bin/env python3
"""Seed a SQLite MVP database with product docs and fake call transcripts."""
...
API_DIR = ROOT / "api"
DEFAULT_DB_PATH = ROOT / "data" / "gtm_copilot_mvp.sqlite3"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed SQLite MVP data for GTM Copilot.")
    parser.add_argument(
        "--db-path",
    ...
    with SessionLocal() as db:
        ...
        def docs_only(since=None):
            ...
            return [
                f
                for f in files
                if f.title.startswith("github/") or f.title.endswith("online-ddl-guidelines.md")
            ]
