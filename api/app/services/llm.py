    ...
    def answer_se_poc_plan(
        ...
        *,
        ...
        prompt = (
            ...
            "1. Review all evidence for: current tech stack, DB pain signals, scale requirements, team maturity, and competitive context.\n"
            "2. Map the target offering to specific TiDB Cloud capabilities (Starter/Essential/Dedicated tier, TiFlash for HTAP, TiCDC for CDC, vector search for AI workloads).\n"
            "3. Identify every gap that would block or delay a successful POC — be specific about what is missing and what action closes it.\n"
            "4. Define success criteria the customer can objectively measure (specific TPS targets, latency p99, migration completeness %, RU cost thresholds — not vague 'performs well').\n"
            "5. Build a week-by-week workplan: Week 1 = setup + baseline, Week 2 = core workload migration/test, Week 3 = scale + performance validation, Week 4 = results review + business case.\n"
            "6. Score readiness 0-100: 80+ = ready, 50-79 = conditional (gaps addressable in <2 weeks), <50 = blocked (fundamental prerequisites missing).\n\n"
            "Return strict JSON with keys:\n"
            "- readiness_score (integer 0-100, scored per rubric above)\n"
            "- readiness_summary (string — 2-3 sentences: overall assessment, the single biggest risk, and whether to proceed now or address gaps first)\n"
            "- gaps (array of strings — each gap must name the specific missing item AND the action to close it, e.g. 'No production workload sample — request 1-week query log export from DBA team')\n"
            "- workplan (array of strings — sequenced week-by-week milestones with specific deliverables, not generic phases)\n"
            "- success_criteria (array of strings — each must be measurable with a specific threshold, e.g. 'P99 read latency <10ms at 5K TPS sustained for 1 hour')\n"
    ...
    def answer_se_poc_readiness(
        ...
        *,
        ...
        prompt = (
            ...
            f"Evidence:\n{context}\n\n"
            "ANALYSIS INSTRUCTIONS:\n"
            "1. Evaluate against the 10-point POC readiness checklist from your system prompt. Score each criterion 0 or 1, then convert to 0-100.\n"
            "2. For each unmet criterion, identify the specific blocker and who needs to take action to resolve it.\n"
            "3. Distinguish hard blockers (cannot start POC without this) from soft blockers (can start but risk poor outcome).\n"
            "4. Required inputs should specify exactly what we need, from whom, and in what format — e.g. 'Query log export (slow_log or general_log, last 7 days) from DBA team lead'.\n"
            "5. Score: 80+ = ready to start, 50-79 = conditional (1-2 hard blockers, addressable in <2 weeks), <50 = blocked (fundamental prerequisites unmet).\n\n"
            "Return strict JSON with keys:\n"
            "- readiness_score (integer 0-100, based on the 10-point checklist evaluation)\n"
            "- readiness_summary (string — 2-3 sentences: what is ready, what is not, and the single most important gap to close before starting)\n"
            "- blockers (array of strings — each must specify: the missing item, why it blocks the POC, who owns resolving it, and suggested timeline. Separate hard blockers from soft blockers with prefix '[HARD]' or '[SOFT]')\n"
            "- required_inputs (array of strings — each must name: the specific artifact needed, who provides it, format/scope, and why the POC cannot produce valid results without it)\n"
            "- status (string: ready|conditional|blocked)\n"
        )
        llm = self._responses_json(system_prompt, prompt, model=model)
        if not isinstance(llm, dict):
    ...
    def answer_se_architecture_fit(
        ...
        *,
        ...
        prompt = (
            ...
            f"Evidence:\n{context}\n\n"
            "ANALYSIS INSTRUCTIONS:\n"
            "1. Identify their current primary database(s) and infrastructure from evidence. Flag assumptions that need customer confirmation.\n"
            "2. Assess TiDB fit across 5 dimensions: MySQL compatibility (wire protocol, SQL dialect, driver compatibility), horizontal scale (write throughput, sharding elimination), HTAP opportunity (TiFlash for analytics on live data), AI/vector fit (embedding storage, hybrid relational+vector queries), and operational simplification (consolidating multiple systems).\n"
            "3. For each strong_fit_for item: name the specific workload or use case AND the TiDB capability that addresses it — e.g. 'Real-time fraud scoring — TiFlash MPP eliminates ETL to separate analytics store, sub-second aggregations on live transaction data'.\n"
            "4. For watchouts: be technically precise about TiDB behavior differences — AUTO_INCREMENT semantics, stored procedure limitations, full-text search gaps, transaction size limits. Include mitigation for each.\n"
            "5. For migration_path: provide ordered, actionable steps with specific tools — e.g. 'Phase 1: Run TiDB Lightning in logical mode to import production schema + sample data (est. 2 hours for 500GB)'.\n"
            "6. Rate overall migration complexity: Low (MySQL-compatible, <1 week), Medium (schema changes needed, 2-4 weeks), High (significant app changes, >1 month). State the rating and top 3 reasons.\n\n"
            "Return strict JSON with keys:\n"
            "- fit_summary (string — 3-4 sentences: overall architecture fit assessment, migration complexity rating with rationale, and the single strongest TiDB value proposition for this account)\n"
            "- strong_fit_for (array of strings — each entry names a specific workload + the TiDB capability that solves it + why their current solution falls short)\n"
            "- watchouts (array of strings — each entry names a specific compatibility risk or TiDB limitation + severity [Low/Medium/High] + mitigation approach)\n"
            "- migration_path (array of strings — ordered steps with specific tools, estimated timelines, and prerequisites for each phase)\n"
        )
        llm = self._responses_json(system_prompt, prompt, model=model)
        if not isinstance(llm, dict):
            return None
    ...
    def answer_se_competitor_coach(
        ...
        *,
        ...
        prompt = (
            ...
            f"Evidence:\n{context}\n\n"
            "ANALYSIS INSTRUCTIONS:\n"
            f"1. Identify {competitor}'s specific strengths and weaknesses relative to TiDB for THIS account's use case — not generic comparisons.\n"
            "2. For positioning: lead with the customer's pain, then why TiDB solves it better. Format: 'Their pain: [X] → TiDB advantage: [Y] → Competitor gap: [Z]'.\n"
            "3. For proof_points: cite specific, verifiable evidence — benchmark results (link or source), named customer references in similar verticals, Jepsen test results, specific performance numbers. No vague claims.\n"
            f"4. For landmines: questions to ask the customer that expose {competitor}'s architectural limitations. Frame as genuine discovery, not leading questions. Include the expected answer and why it matters.\n"
            "5. For discovery_questions: MEDDPICC-aligned questions that simultaneously qualify the deal AND position TiDB favorably. Each question should include the business reason it matters.\n\n"
            "COMPETITIVE KNOWLEDGE TO APPLY:\n"
            "- vs CockroachDB: TiDB MySQL compatibility (zero app changes for MySQL shops), TiFlash HTAP (CockroachDB has no columnar engine), better ecosystem tooling (TiCDC, TiDB Lightning)\n"
            "- vs PlanetScale: TiDB is self-hostable + managed, supports HTAP, no branch-based workflow lock-in, vector search capability\n"
            "- vs Aurora: TiDB horizontal write scaling (Aurora scales reads only), TiDB HTAP eliminates separate analytics store, multi-cloud portability\n"
            "- vs Vitess: TiDB is native distributed SQL (no middleware layer), simpler operations, built-in HTAP, no application-level sharding awareness needed\n"
            "- vs YugabyteDB: TiDB MySQL wire compatibility (Yugabyte is PostgreSQL), TiFlash columnar engine for analytics, larger production deployment base in APAC\n"
            "- vs SingleStore: TiDB stronger OLTP performance, MySQL compatibility, lower TCO at scale, open-source foundation\n\n"
            "Return strict JSON with keys:\n"
            f"- competitor (string — full name of {competitor})\n"
            "- positioning (array of strings — each entry: 'Customer pain → TiDB advantage → Competitor gap' format, specific to this account's use case)\n"
            "- proof_points (array of strings — each must cite a specific, verifiable source: benchmark URL, customer name, Jepsen result, or documented capability)\n"
            "- landmines (array of strings — each: a discovery question that exposes a competitor limitation + the expected answer + why it matters for this deal)\n"
            "- discovery_questions (array of strings — each: the question + which MEDDPICC element it qualifies + the business reason it matters)\n"
        )
        llm = self._responses_json(system_prompt, prompt, model=model)
        if not isinstance(llm, dict):
            return None
    ...
    def answer_marketing_intelligence(
        ...
        *,
        ...
        prompt = (
            ...
            f"Verticals: {', '.join(verticals) if verticals else 'All'}\n"
            f"Evidence:\n{context}\n\n"
            "ANALYSIS INSTRUCTIONS:\n"
            "1. Analyze evidence for demand signals: hiring patterns (DB/infra roles = active investment), funding events (budget trigger), tech stack signals (MySQL/Aurora at scale = TiDB fit), competitive moves (evaluating distributed SQL), and growth indicators (scaling pain imminent).\n"
            "2. Map every signal to a pipeline funnel stage: MQL (awareness), SQL (consideration), SAL (evaluation), Opportunity (closing). Different signals require different content.\n"
            "3. For campaign_angles: frame each angle in the target vertical's language. Fintech = compliance + write scale + audit. Ad-tech = throughput + real-time analytics + cost per event. SaaS = multi-tenant scale + operational simplicity. Gaming = global latency + consistency.\n"
            "4. For priority_accounts: rank by signal strength. Cite the specific signal source for each account.\n"
            "5. For next_actions: every action must have an owner, a deliverable, a funnel stage target, and a measurable success metric (e.g. 'Content team: Publish MySQL-to-TiDB migration guide targeting SAL stage — measure by demo request conversion rate from guide page').\n"
            "6. If evidence is limited (<5 accounts or signals), explicitly state confidence limits and what additional data would improve the analysis.\n\n"
            "CONTENT-TO-SIGNAL MAPPING:\n"
            "- Hiring signal → content about operational complexity reduction (fewer DBAs, automated scaling)\n"
            ...
            "- AI/ML signal → TiDB vector search + HTAP for feature stores content\n\n"
            "Return strict JSON with keys:\n"
            "- summary (string — 3-4 sentences: market landscape for these regions/verticals, strongest aggregate signal pattern, and the single highest-leverage campaign opportunity)\n"
            "- top_signals (array of strings — each: 'Signal type: [specific evidence] → Funnel stage: [MQL/SQL/SAL/Opp] → Recommended action: [specific content or campaign]')\n"
            "- campaign_angles (array of strings — each: the angle framed in vertical-specific language + target funnel stage + content type + measurable success metric)\n"
            "- priority_accounts (array of strings — each: 'Account name — [primary signal with source] — [recommended entry point: role + angle]')\n"
            "- next_actions (array of strings — each: 'Owner: [specific action] targeting [funnel stage] — success metric: [measurable outcome] — timeline: [date]')\n"
        )
        llm = self._responses_json(system_prompt, prompt, model=model)
        if not isinstance(llm, dict):
            return None
