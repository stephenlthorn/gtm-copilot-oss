...
def test_sync_feishu_oauth_mode_requires_connected_user_token(client):
    ...
    res = client.post("/admin/sync/feishu", headers={"X-User-Email": "rep@example.com"})
    assert res.status_code == 200
    data = res.json()
...
def test_sync_feishu_oauth_mode_uses_user_access_token(client, monkeypatch):
    ...
    res = client.post("/admin/sync/feishu", headers={"X-User-Email": "se@example.com"})
    assert res.status_code == 200
    data = res.json()

    assert data["status"] == "ok"
    assert called["token_args"]["user_email"] == "se@example.com"
    assert called["init_kwargs"]["access_token"] == "user-token"
    assert called["init_kwargs"]["user_email"] == "se@example.com"
    assert called["roots"] == ["root_a"]
    assert called["synced_user"] == "se@example.com"
