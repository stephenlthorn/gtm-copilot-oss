...
def test_slack_command_calls_oracle_and_returns_formatted_reply(client, monkeypatch):
    ...
    def fake_run(self, *, mode, user, message, top_k, filters, context):
        ...
        return (
            {
                "answer": "Replica Analytics lag depends on write pressure and replica topology.",
                "citations": [
                    {
                        "title": "Replica Analytics Replica Docs",
                        "source_id": "doc_123",
                        "chunk_id": "chunk_abc",
                    }
                ],
    ...
    body = (
        "command=/gtm-copilot&text=What+is+Replica Analytics+replication+lag%3F"
    ...
    assert "Replica Analytics lag depends" in payload["text"]
...
def test_slack_command_rejects_invalid_signature(client):
    ...
    body = b"command=/gtm-copilot&text=hello"
    headers = {
