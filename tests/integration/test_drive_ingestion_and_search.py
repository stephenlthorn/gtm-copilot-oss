...
def test_ingest_50_plus_docs_and_search_citations(db_session, tmp_path):
    ...
    for i in range(1, 56):
        (fake_drive / f"doc_{i:02d}.md").write_text(
            f"# Doc {i}\n\nDistributed SQL online DDL guidance for account {i}.\n\n## Notes\nSchema changes and scaling practices.",
            encoding="utf-8",
        )
...
def test_search_viewer_filter_allows_google_shared_drive_docs_without_user_tag(db_session):
    ...
    doc_viewer = KBDocument(
        ...
        tags={"user_email": "rep@example.com"},
    )
    doc_other = KBDocument(
        ...
        tags={"user_email": "other@example.com"},
    )
    ...
    hits = retriever.search(
        ...
        top_k=10,
        filters={"source_type": ["google_drive"], "viewer_email": "rep@example.com"},
    )
...
def test_kb_routes_allow_google_shared_drive_docs_without_user_tag(client, db_session):
    ...
    headers = {"X-User-Email": "rep@example.com"}
...
def test_drive_ingestor_user_scoped_sync_does_not_fallback_to_service_account(db_session, monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        ingestor.sync(user_email="rep@example.com")
